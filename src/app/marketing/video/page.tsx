...
import { brandStore, videoStore, settingsStore, preferenceStore } from "@/lib/store";
...
const AI_PROVIDERS = ["Claude", "GPT-4", "Gemini"] as const;
  ...
  const exportScript = useCallback(() => {
    ...
    const fullScript = [
      ...
      ...script.scenes.map((s) => [
        ...
        ...(s.thumbnailIdea ? [`THUMBNAIL: ${s.thumbnailIdea}`] : []),
        "",
      ].join("\n")),
      `CTA: ${script.cta}`,
      ...(script.thumbnailIdea ? ["", `FINAL THUMBNAIL IDEA: ${script.thumbnailIdea}`] : []),
    ].join("\n");
  ...
  return (
    <div className="min-h-screen bg-gray-950 p-8 space-y-6">
      <div>
        ...
        <p className="text-gray-400 mt-1">Create video scripts and storyboards powered by brand intelligence</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-pink-600 resize-none"
              rows={3}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-pink-600"
            />
          </div>
            ...
            {[
              ...
              <div key={field.label}>
                ...
                <select
                  ...
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-pink-600"
                >
        ...
        {/* Output Panel */}
        <div>
          {error && <div className="bg-red-500/20 text-red-400 text-sm px-4 py-3 rounded-lg mb-4">{error}</div>}
          {script ? (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5 overflow-y-auto max-h-[800px]">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Duration: {script.totalDuration} · Music: {script.music}
                  </p>
                </div>
              </div>

              {/* Hook Box */}
              <div className="bg-pink-500/20 border-2 border-pink-600 rounded-lg p-4">
                <p className="text-xs font-bold text-pink-400 mb-2">HOOK</p>
                <p className="text-base font-semibold text-white">{script.hook}</p>
              </div>

              {/* Scenes */}
              <div className="space-y-3">
                {script.scenes.map((scene) => (
                  <div key={scene.sceneNumber} className="bg-gray-800 border border-gray-700 rounded-lg p-4 space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-bold text-pink-400">SCENE {scene.sceneNumber}</span>
                    ...
                    {scene.transition && (
                      <div>
                        ...
                        <p className="text-sm text-gray-300">{scene.transition}</p>
                      </div>
                    )}
                    {scene.thumbnailIdea && (
                      <div>
                        <p className="text-xs text-gray-500 mb-1">THUMBNAIL IDEA</p>
                        <p className="text-sm text-gray-300 italic">{scene.thumbnailIdea}</p>
                      </div>
                    )}
                  </div>
                ))}
              ...
              {script.thumbnailIdea && (
                <div className="bg-pink-500/20 border border-pink-500/30 rounded-lg p-4">
                  <p className="text-xs font-bold text-pink-400 mb-2">FINAL THUMBNAIL IDEA</p>
                  <p className="text-sm text-white">{script.thumbnailIdea}</p>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-700">
                ...
                <button
                  onClick={handleSave}
                  className="flex-1 bg-pink-600 text-white font-medium py-2.5 px-4 rounded-lg hover:bg-pink-500 transition-colors text-sm"
                >
                  Save Script
                </button>
              </div>
